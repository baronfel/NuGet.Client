...
namespace NuGet.ProjectModel
{
    public static partial class PackagesLockFileFormat
    {
            ...
            out string? value)
        {
            switch (reader.TokenType)
            {
                ...
                case JsonTokenType.Null:
                    value = reader.ReadScalarAsInvariantString();
                    return true;
                default:
                    reader.Skip();
                    value = null;
        ...
        private static Stream CreateUtf8Stream(Stream stream)
        {
            return new TextReaderUtf8Stream(new PrefixReplayStream(stream));
        }
...
#pragma warning disable CA2213 // The outer package-lock reader owns the source stream.
...
#pragma warning restore CA2213
            ...
            internal PrefixReplayStream(Stream stream)
            {
                _stream = stream;
                _prefix = new byte[PrefixSize];

                while (_prefixPosition < _prefix.Length)
                {
                    int bytesRead = stream.Read(_prefix, _prefixPosition, _prefix.Length - _prefixPosition);
                    if (bytesRead == 0)
                    {
                        break;
                    }

                    _prefixPosition += bytesRead;
                }

                PrefixLength = _prefixPosition;
                _prefixPosition = 0;
            }
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                ...
                int prefixBytesRead = Math.Min(count, PrefixLength - _prefixPosition);
        ...
        private sealed class TextReaderUtf8Stream : Stream
        {
            ...
            internal TextReaderUtf8Stream(Stream stream)
            {
                ...
                _bytes = new byte[Encoding.UTF8.GetMaxByteCount(CharBufferSize)];
            }
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                if (_disposed)
                {
                    throw new ObjectDisposedException(nameof(TextReaderUtf8Stream));
                }
            ...
            private void FillEncodedBuffer()
            {
                do
                {
                    int charsRead = _reader.Read(_chars, index: 0, count: _chars.Length);
