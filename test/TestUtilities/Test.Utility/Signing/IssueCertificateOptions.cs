...
namespace Test.Utility.Signing
{
    public sealed class IssueCertificateOptions
    {
        ...
        public static IssueCertificateOptions CreateDefaultForRootCertificateAuthority()
        {
            ...
            var subjectName = new X500DistinguishedName($"CN=NuGet Test Root Certificate Authority ({id}),O=NuGet,L=Redmond,ST=WA,C=US");
        ...
        public static IssueCertificateOptions CreateDefaultForIntermediateCertificateAuthority()
        {
            var keyPair = CertificateUtilities.CreateKeyPair();
            var id = CertificateUtilities.GenerateRandomId();
            var subjectName = new X500DistinguishedName($"CN=NuGet Test Intermediate Certificate Authority ({id}),O=NuGet,L=Redmond,ST=WA,C=US");
        ...
        public static IssueCertificateOptions CreateDefaultForEndCertificate()
        {
            var keyPair = CertificateUtilities.CreateKeyPair();
            var id = CertificateUtilities.GenerateRandomId();
            var subjectName = new X500DistinguishedName($"CN=NuGet Test Certificate ({id}),O=NuGet,L=Redmond,ST=WA,C=US");
        ...
        public static IssueCertificateOptions CreateDefaultForTimestampService()
        {
            ...
            var subjectName = new X500DistinguishedName($"CN=NuGet Test Timestamp Service ({id}),O=NuGet,L=Redmond,ST=WA,C=US");
