    ...
    public class AccuracyTests
    {
        ...
        private static byte[] GetAccuracyBytes(int? seconds, int? milliseconds, int? microseconds)
        {
            ...
            using (writer.PushSequence())
            {
                ...
                if (milliseconds is not null)
                {
                    writer.WriteInteger((long)milliseconds, new Asn1Tag(TagClass.ContextSpecific, 0));
                }

                if (microseconds is not null)
                {
                    writer.WriteInteger((long)microseconds, new Asn1Tag(TagClass.ContextSpecific, 1));
                }
            }
